...
class RiskManagementAgent:
    ...
    def _check_daily_loss_limit(self) -> Tuple[bool, str]:
        ...
        if daily_loss_pct >= self.limits.max_daily_loss_pct:
            return (
                False,
                f"DAILY_LOSS_LIMIT_HIT {daily_loss_pct:1.1f}% >= {self.limits.max_daily_loss_pct}%",
            )
        return True, ""
    ...
    def _check_weekly_loss_limit(self) -> Tuple[bool, str]:
        ...
        if weekly_loss_pct >= self.limits.max_weekly_loss_pct:
            return (
                False,
                f"WEEKLY_LOSS_LIMIT_HIT {weekly_loss_pct:1.1f}% >= {self.limits.max_weekly_loss_pct}%",
            )
        return True, ""
    ...
    def _check_max_drawdown(self) -> Tuple[bool, str]:
        ...
        if drawdown >= self.limits.max_drawdown_pct:
            return (
                False,
                f"MAX_DRAWDOWN_HIT {drawdown:1.1f}% >= {self.limits.max_drawdown_pct}%",
            )
        return True, ""
    ...
    def _check_position_count(self) -> Tuple[bool, str]:
        if len(self.state.open_positions) >= self.limits.max_position_count:
            return (
                False,
                f"MAX_POSITION_COUNT_HIT {len(self.state.open_positions)} >= {self.limits.max_position_count}",
            )
        return True, ""
    ...
    def _check_reward_risk_ratio(self, trade: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if rr < self.limits.min_reward_risk_ratio:
            return (
                False,
                f"REWARD_RISK_TOO_LOW {rr:.2f} < {self.limits.min_reward_risk_ratio}",
            )
        return True, ""
    ...
    def _size_position(self, trade: Dict[str, Any]) -> Dict[str, Any]:
        ...
        portfolio_cap = (
            ...
            / 100
            / max(len(self.state.open_positions), 1)
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "drawdown_pct": (self.state.peak_capital - self.state.capital)
            / self.state.peak_capital
            * 100,
