...
class APEXOrchestrator:
    ...
    def __init__(self):
        ...
        self.risk_manager = RiskManager(
            config=self.config, redis_client=self.redis, executor=self.executor
        )
    ...
    async def monitor_and_act(self):
        ...
        while self.running:
            try:
                ...
                if open_positions:
                    unsafe_exits = await self.master_decision.monitor_open_positions(
                        open_positions, market_data
                    )
                    for exit_info in unsafe_exits:
                        log.warning(
                            f"AUTO-EXIT: Disposed unsafe position {exit_info['symbol']} | {exit_info['reason']}"
                        )
                        # 1. Close in local Portfolio
                        # In a real environment, we'd fetch the current LTP from feed
                        exit_price = 0.0  # TODO: Get LTP
                        self.portfolio_manager.close_position(
                            exit_info["position_id"], exit_price
                        )
                ...
                if decision.final_direction != SignalDirection.NEUTRAL:
                    ...
                    proposed = {
                        ...
                        if decision.final_direction == SignalDirection.BULLISH
                        else market_data.get("price", 0) * 1.02,
                    }

                    approved, reason, trade = self.risk_manager.validate_signal(
                        {}, proposed
                    )

                    if approved and trade["quantity"] > 0:
                        log.info(
                            f"LIVE SIGNAL APPROVED: {trade['symbol']} {trade['direction']} | Qty: {trade['quantity']} | Reason: {decision.reasoning}"
                        )
                        ...
                        if trade.get("quantity") == 0:
                            log.debug(
                                f"Trade suppressed: Risk limit or insufficient funds for {proposed['symbol']}"
                            )
                        else:
    ...
    async def start(self):
        ...
        log.info(
            f"APEX ONLINE | {len(self.agents)} Agents Active | Decision Loop Running"
        )
