...
- description: Generate new options trade signals based on current regime
  ...
  format_guide: 'Read MARKET_STATE from APEX_TRADING memory (). Based on the
    ...
    memory under TRADE_SIGNAL_N keys. Only generate signals if regime warrants it
...
- description: Risk veto — filter and approve signals
  ...
  format_guide: 'Read all new TRADE_SIGNALs and PAPER_STATS from APEX_TRADING memory
    ...
    signal with reason. Write VETO_REPORT and update APPROVED_SIGNALS in memory. Output:
    approved signals, rejected signals with reasons, current risk exposure.'
- description: Execute approved signals via Dhan paper trade engine
  ...
  format_guide: 'Read APPROVED_SIGNALS from APEX_TRADING memory (). For each
    ...
    to memory. Output: new fills, open positions, current MTM P&L.'
...
Runs every 15 minutes during NSE market hours. Regime engine classifies market state, options strategy engine generates new signals, risk veto filters them, paper trade engine executes approved ones.
